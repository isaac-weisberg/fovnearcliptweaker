...
class NearClipControlWidget:
	def __init__(self, app, pos, height):
		self.pos = pos
		self.height = height
		minX = pos.x
		self.currentValue = 0
		...
		self.controlButtonNone = Button(app, 'No Control', vec2(minX, pos.y), vec2(160, height))
		self.controlButtonNone.setDrawBorder(0)
		self.controlButtonNone.setBackgroundColor(accentColor)
		self.controlButtonFollowDistance = Button(app, 'Follow Distance', vec2(self.controlButtonNone.maxX() + 16, pos.y), vec2(200, height))
